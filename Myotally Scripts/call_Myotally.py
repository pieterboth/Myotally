...
def main(folder, DAPI_channel, Lam_channel, mfi_channels, IQ, mia, MA):
    ...
    results.write('\nFiber Size Range (pixels): ' + str(mia) + ' - ' + str(MA))
    results.write('\n' +'Image'.rjust(50) + '\t' + 'Number of Fibers'.rjust(20) + '\t' + 'Average CSA (pixels)'.rjust(20))
        ...
        if path == folder:
            ...
            for d in direcs:
                ...
                if d != '.DS_Store' and len(set(keep_files)&set(os.listdir(folder+'/'+d))) == len(set(keep_files)):
                    ...
                    results.write(str(d).rjust(50) + '\t' + str(len(fibers)).rjust(20) + '\t' + str(format(np.mean(CSAs),'.1f')).rjust(20))
                    ...
                    for f in fibers:
                        current.write('\n'+str(f.number).rjust(20)+'\t'+str(f.area).rjust(20)+'\t'+str(f.CNF).rjust(20))
                        for i,b in enumerate(mfi_channels):
